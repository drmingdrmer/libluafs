...
static int pChmod(lua_State *L)
{
    ...
    if (! lua_isstring(L, 1))
        return _return(L, "chmod(string,int);");
    ...
    if (mode < 0)   /* Make sure the octal conversion succeeded */
        return _return(L, "invalid mode for chmod()");
...
static int pChown(lua_State *L)
{
    ...
    if (lua_isstring(L, 1))
        ...
        return( _return(L, "chown(string,[string|int],[string|int]);" ) );
...
static int pIsDir(lua_State *L)
{
    ...
    if (lua_isstring(L, 1))
        ...
        return( _return(L, "is_dir(string)" ) );
...
static int pIsFile(lua_State *L)
{
    ...
    if (lua_isstring(L, 1))
        ...
        return( _return(L, "is_file(string)" ) );
...
/**
 * Test to see if the given file/path whether exist.
 * @param L The lua intepreter object.
 * @return The number of results to be passed back to the calling Lua script.
 */

static int pIsExist(lua_State *L)
{
    const char *fileName = NULL;
    ...
    if (lua_isstring(L, 1))
        ...
        return( _return(L, "is_exist(string)" ) );
...
/**
 * Perform a makedir() operation.
 * @param L The lua intepreter object.
 * @return The number of results to be passed back to the calling Lua script.
 */

static int pMakedir(lua_State *L)
{
    const char *path;
    int mode;
    ...
    if (lua_isstring(L, 1))
        ...
        return( _return(L, "makedir(string,int);" ) );
    ...
    if (mode < 0)   /* Make sure the octal conversion succeeded */
        return _return(L, "invalid mode for makedir()");

    if (mkdir(path,mode) != 0)
        return(pusherror(L, "Failed to makedir()" ) );
...
static int pReadDir(lua_State *L)
{
    ...
    if (lua_isstring(L, 1))
        ...
        return( _return(L, "readdir(string)" ) );
...
/**
 *modify file atime,mtime
 *the first param is filename
 *the second param is atime
 *the third param is mtime
 *@return The number of results to be passed back to the calling Lua script.
 */

static int pMtime(lua_State *L)
{
    const char *path;
    struct utimbuf mtime;

    if (! lua_isstring(L, 1))
        return _return(L, "utime(string,long,long);");
    ...
    mtime.actime = lua_tonumber( L, 2 );

    if (! lua_isnumber(L, 3))
        return _return(L, "utime(string,long,long);");

    mtime.modtime = lua_tonumber( L, 3 );

    if ( utime( path,&mtime )!= 0 )
        return pusherror(L, "cannot modify file time utime()");
...
LUALIB_API int luaopen_libluafs(lua_State *L)
{
